...
@transaction.atomic
def create_sale(user, company, fy, party, date, lines, number=None, segregate_flag=None):
    ...
    for ln in lines:
        item_id = ln["item"]
        # resolve company: explicit per-line, else master company / default
    ...
    post_entry(party, date, fy, "SALE", master.id, debit=total)
