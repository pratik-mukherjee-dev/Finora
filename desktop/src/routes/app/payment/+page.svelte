<script lang="ts">
    ...
    // ── settlement category: Cash vs Bank ─────────────────────────────────────
    ...
    // Cash/Bank category + bank sub-mode + reference
...
<div class="wrap" use:enterFlow={flowOpts}>
    ...
    <p class="sub">Pay a vendor. Auto-settles open purchases oldest → latest.</p>
    ...
    <!-- Cash / Bank toggle + sub-mode + reference -->
    <section class="mode-section">
        ...
        {#if settleCategory === "BANK"}
            ...
            <div class="field ref">
                <label for="txn-ref">
                    {bankSubMode === "CHEQUE" ? "Cheque No." : bankSubMode === "UPI" ? "UPI Ref / UTR" : "Transaction ID"}
                </label>
    ...
    <footer class="foot">
        <button class="save" data-flow="save" disabled={!canSave} onclick={requestSave} type="button">
            {saving ? "Saving…" : "Save payment"} <kbd>Ctrl ⏎</kbd>
        </button>
    </footer>
</div>
