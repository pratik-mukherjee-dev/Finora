<script lang="ts">
    ...
    // ── settlement row (one per mode split) ───────────────────────────────────
    type SettleRow = {
        key: number;
        modeId: number | null;
        amount: string;
    ...
    const endpoint = "/api/vouchers/payments/";
    ...
    // Category options for the LedgerLookup-style dropdown
    const categoryOptions = [{id: 1, name: "Cash"}, {id: 2, name: "Bank"}];
    const bankTypeOptions = [
        {id: 1, name: "UPI"}, {id: 2, name: "Transfer"},
        {id: 3, name: "Cheque"}, {id: 4, name: "Other"},
    ];

    // Helpers to get mode metadata for a row
    ...
    function categoryOf(row: SettleRow): string {
        return modeOf(row)?.category ?? "";
    }

    function bankTypeOf(row: SettleRow): string {
        return modeOf(row)?.bank_type ?? "";
    }
    ...
    function collectIssues(): Issue[] {
        ...
        rows.forEach((r, i) => {
            ...
            if (amt > 0 && r.modeId == null) {
                issues.push({
                    code: `row-${i}-no-mode`,
                    message: `Row ${i + 1}: amount entered but no mode selected.`,
            ...
            if (amt > 0 && mode?.category === "BANK" && mode?.bank_type === "CHEQUE" && !r.transactionRef.trim()) {
                issues.push({
                    code: `row-${i}-no-cheque`,
                    message: `Row ${i + 1} (${mode.name}): cheque number is empty. You can proceed without it.`,
    ...
    // Enter on amount: if last row, add new row. If blank last row, drop it and go to save.
    ...
    // After mode is picked: if BANK mode, flow stays in row (amount next via normal flow).
    // The LedgerLookup onenter fires after pick, which advances to amount via data-flow.
    function onModePicked(row: SettleRow) {
        // Normal flow: LedgerLookup's onenter will advance to next data-flow element (amount).
        // Nothing extra needed here.
    }

    // Empty mode enter on last blank row: drop and go to save
    function onModeEmptyEnter(row: SettleRow) {
        const isLast = rows[rows.length - 1]?.key === row.key;
        const blank = row.modeId == null && (Number(row.amount) || 0) === 0;
        if (isLast && rows.length > 1 && blank) {
            removeRow(row.key);
        }
        setTimeout(() => document.querySelector<HTMLElement>('[data-flow="save"]')?.focus(), 0);
    }
    ...
    // ── save: one POST per row, sequentially ──────────────────────────────────
    async function save() {
        ...
        try {
            ...
            for (const r of validRows) {
                ...
                posted.push({
                    ...
                    amount: Number(res.amount), allocations: res.allocations ?? [],
                });
            }
            results = posted;
            rows = [newRow()];
    ...
    const shell = registerScreen(() => ({
        title: "Payment",
        actions: [
            {id: "pay-new", label: "New", icon: "＋", shortcut: "Ctrl+N", run: resetForm},
            {id: "pay-add", label: "Add row", icon: "▸", shortcut: "Alt+A", run: addRow},
            {id: "pay-save", label: "Save", icon: "✓", shortcut: "Ctrl+Enter", run: requestSave},
        ],
        shortcuts: [
            {id: "pay-k-new", keychord: "ctrl+n", label: "New", run: resetForm},
            {id: "pay-k-add", keychord: "alt+a", label: "Add row", run: addRow},
        ],
        panel: [
...
{#snippet allocationPanel()}
    ...
    {:else if selectedResult}
        ...
        {/if}
    {:else if !party}
        <p class="muted">Pick a party to preview open purchases.</p>
        ...
        <div class="preview-summary">
            <div class="ps-row"><span>Balance</span>
                <strong class={bal > 0 ? 'recv' : bal < 0 ? 'pay' : ''}>
                    ...
                    <span class="bal-tag">{bal > 0 ? 'receivable' : bal < 0 ? 'payable' : 'settled'}</span>
                </strong></div>
            {#if onAcc > 0.001}
...
{#snippet historyPanel()}
    <div class="sidehead">
        <span class="muted">Payments{party ? ` · ${party.name}` : ""}</span>
...
<div class="wrap" use:enterFlow={flowOpts}>
    ...
    {#if results.length > 0}
        <div class="banner ok">
            Posted {results.length} payment{results.length > 1 ? "s" : ""} ·
            total {totalAmount.toFixed(2) || results.reduce((s, r) => s + r.amount, 0).toFixed(2)}.
        </div>
    {/if}
    ...
    <section class="grid">
        <div class="ghead">
            <span>Mode</span><span>Amount</span><span>Category</span><span>Bank Mode</span><span>Reference</span><span></span>
        </div>
        {#each rows as row (row.key)}
            ...
            <div class="srow">
                <div class="cell mode-cell">
                    <LedgerLookup flow="mode" options={modeOptions}
                                  value={row.modeId}
                                  placeholder="Pick mode…"
                                  onselect={(id) => (row.modeId = id)}
                                  onenter={() => onModePicked(row)}
                ...
                <div class="cell">
                    <input class="num" type="number" min="0" step="0.01" data-flow="amount"
                           ...
                           onkeydown={(e) => !needsRef(row) ? onAmountEnter(e, row) : undefined}/>
                </div>
                <div class="cell cat-cell">
                    {#if mode}
                        <span class="tag" class:bank={isBank}>{mode.category}</span>
                    {:else}
                        <span class="tag empty">—</span>
                    {/if}
                </div>
                <div class="cell bt-cell">
                    {#if isBank && mode?.bank_type}
                        <span class="tag bank-type">{mode.bank_type}</span>
                    {:else}
                        <span class="tag empty">—</span>
                    {/if}
                </div>
                <div class="cell ref-cell">
                    {#if isBank}
                        <input type="text" data-flow="txn-ref"
                               bind:value={row.transactionRef}
    ...
    <footer class="foot">
        <button class="save" data-flow="save" disabled={!canSave} onclick={requestSave} type="button">
            {saving ? "Saving…" : "Post payment"} <kbd>Ctrl ⏎</kbd>
        </button>
    </footer>
</div>
...
{/if}
{#if confirmOpen}
    <ConfirmDialog title="Confirm payment"
                   message={confirmMessage}
                   confirmLabel="Post payment" busy={saving}
