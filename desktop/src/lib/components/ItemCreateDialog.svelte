...
<div class="backdrop" onmousedown={oncancel}>
  <form class="dialog" onmousedown={(e) => e.stopPropagation()} onsubmit={save}>
    <h2>New item</h2>
    <p class="muted">Mapped to current company.</p>
    <input bind:value={name} placeholder="Item name" autofocus />
...
<style>
  .backdrop { position: fixed; inset: 0; background: rgba(0,0,0,.55); display: grid; place-items: center; z-index: 50; }
  .dialog { display: flex; flex-direction: column; gap: 10px; width: 340px; padding: 22px;
            background: #171a21; border-radius: 12px; color: #e6e8ec; }
  h2 { margin: 0; font-size: 17px; }
  .muted { color: #9aa0aa; font-size: 12px; margin: 0; }
  input { padding: 9px 11px; border-radius: 8px; border: 1px solid #2a2f3a; background: #0f1115; color: #e6e8ec; }
  .grid { display: grid; grid-template-columns: 1fr 1fr; gap: 8px; }
  .row { display: flex; justify-content: flex-end; gap: 8px; margin-top: 4px; }
  button { padding: 8px 14px; border-radius: 8px; border: 1px solid #2f6feb; background: #2f6feb; color: #fff; cursor: pointer; }
